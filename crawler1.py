...
# Always fetch after selection (simple/robust)
...
# Reorder to a clean schema (we'll hide some in the display below)
preferred = [
...
# Keep a date key (for striping) but we'll drop 'date' from visible table
date_key = pd.to_datetime(df["date"], errors="coerce").dt.strftime("%Y-%m-%d").fillna(df["date"].astype(str))

# Create the display dataframe with columns hidden/removed
df_display = df.drop(columns=["date", "bodyweight", "internal_weight", "percentile"], errors="ignore")

# --- alternating row striping by date (softer bg + dark text) ---
groups = (date_key != date_key.shift(1)).cumsum()

def _stripe_rows_by_date(df_shown: pd.DataFrame):
    styles = pd.DataFrame("", index=df_shown.index, columns=df_shown.columns)
    # restrict groups to visible index order
    gvis = groups.loc[df_shown.index]
    for g in gvis.unique():
        idx = df_shown.index[gvis == g]
        color = "#f0f2f6" if (g % 2 == 1) else "#fafbfc"   # both are soft; second is off-white
        styles.loc[idx, :] = f"background-color: {color}; color: #111;"
    return styles

# Slightly softer gridlines (optional)
st.markdown("""
<style>
...
# Show the striped, cleaned table (hidden cols removed)
st.dataframe(
    df_display.style.apply(_stripe_rows_by_date, axis=None),
...
# Download: keep the full dataset (including hidden cols) so you have everything
st.download_button(
